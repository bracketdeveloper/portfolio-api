...
class ProjectCategoryController {
    ...
    private function getAll() {
        ...
        // PDO::FETCH_ASSOC ensures id and sort_order return alongside name
        $categories = $stmt->fetchAll(PDO::FETCH_ASSOC);
        
        // Force all values to string data types to match your specification
        foreach ($categories as &$category) {
            $category['id'] = (string)$category['id'];
            $category['sort_order'] = (string)$category['sort_order'];
        }
    ...
    private function create() {
        ...
        if ($stmt->execute()) {
            ...
            echo json_encode(["message" => "Category created successfully.", "id" => (string)$this->db->lastInsertId()]);
        } else {
